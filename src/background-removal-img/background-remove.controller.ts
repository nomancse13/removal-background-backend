...
export class BackgroundRemovalController {
  constructor(private readonly backgroundRemovalService: BackgroundRemovalService) {}
...
@ApiOperation({
  ...
  description: 'this route is responsible for removing background of an image',
})
  @Post('upload/remove')
  @UseInterceptors(
  ...
  async removeBackgroundDemo(@UploadedFile() file: Express.Multer.File, @UserPayload() user: UserInterface
  ) {
    const inputImagePath = file.path;
    const outputImagePath = Date.now() + '.jpg'; // Provide the desired output path
  ...
  async fileUpload(@UploadedFile() file: Express.Multer.File, @UserPayload() user: UserInterface
  ) {
