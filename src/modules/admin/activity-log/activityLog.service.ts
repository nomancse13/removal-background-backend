...
import { CreateActivityLogDto,} from './dtos';
...
export class ActivityLogService {
  ...
  //   create plan
  async entryLog(createActivityLogDto: CreateActivityLogDto, userPayload?: UserInterface) {
    createActivityLogDto['createdBy'] = userPayload? userPayload.id: 0;
    createActivityLogDto['userId'] = userPayload? userPayload.id: 0;
    // save data
    const data = await this.activityLogRepository.save(createActivityLogDto);
    return data;
  }
  ...
  async getLogCount(ipAddress: string) {
    const data = await this.activityLogRepository.createQueryBuilder('log')
