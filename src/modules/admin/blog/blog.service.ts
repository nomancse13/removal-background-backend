...
import { ErrorMessage, StatusField, SuccessMessage, UserTypesEnum } from 'src/authentication/common/enum';
...
export class BlogService {
  ...
  async updateBlog(
    ...
    // if (updateDto.status === StatusField.DELETED) {
    //   updateDto['status'] = updateDto.status;
    //   updateDto['deletedAt'] = new Date();
    //   updateDto['deletedBy'] = id;
    //   updateDto['updatedBy'] = null;
    //   updateDto['updatedType'] = null;
    // } else {
    //   updateDto['status'] = updateDto.status;
    //   updateDto['updatedAt'] = DateTime();
    //   updateDto['updatedBy'] = id;
    //   updateDto['updatedType'] = userTypeSlug;
    //   updateDto['deletedAt'] = null;
    //   updateDto['deletedBy'] = null;
    // }
   ...
   * GET LIST -> Blog
   */
  async filterBlog( listQueryParam: PaginationOptionsInterface,
    filter: any, userPayload: UserInterface) {
      
    if (decrypt(userPayload.hashType) !== UserTypesEnum.ADMIN) {
     throw new BadRequestException('You are not allow to see any kind of Blog')
    }
   ...
   */
    ...
    async deleteBlog(id: number, userPayload: UserInterface) {
      ...
      if (decrypt(userPayload.hashType) !== UserTypesEnum.ADMIN) {
        throw new BadRequestException('You are not allow to delete any kind of blog')
       }
  ...
  async getBlogPage() {
    ...
    return getLatestFiveBlog ? getLatestFiveBlog : ErrorMessage.RECORD_NOT_FOUND;
  }
